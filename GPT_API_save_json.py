...
def save_response_text(response_text, filename_prefix="response"):
    ...
    filename = f"{directory}/{filename_prefix}_{current_time}.json"
    with open(filename, "w") as file:
        json.dump({"response": response_text}, file)
    return filename

response_text_tool = FunctionTool.from_defaults(
    fn=save_response_text,
    name="response_text_saver",
    description="Saves the response text to a JSON file in the temp directory."
)
...
def process_inquiry():
    ...
    return jsonify({"response": saved_response["response"], "file_path": response_file})
